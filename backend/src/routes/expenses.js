...
router.post('/', checkRegisterLock, async (req, res) => {
  try {
    const {
      category, sub_category, description, amount, payment_method,
    ...
    const result = await query(
      ...
      [
        category || null, sub_category || null, description, amount,
        payment_method || 'cash',
...
router.put('/:id', async (req, res) => {
  try {
    ...
    const result = await query(
      `UPDATE expenses SET
        category=$1, sub_category=$2, description=$3, amount=$4, payment_method=$5,
