...
function printInvoice(sale) {
  ...
  const html = `
    <!DOCTYPE html><html><head><title>Invoice ${sale.invoice_number}</title>
    <style>
      ...
      .shop-name{font-size:24px;font-weight:bold;color:#1a1a2e}.shop-sub{font-size:12px;color:#6b7280;margin-top:4px}
      .invoice-title{font-size:28px;font-weight:bold;color:#6366f1}.invoice-meta{font-size:13px;color:#6b7280;text-align:right}
    ...
    <div class="header">
      <div>
        <div class="shop-name">${sale.shop_name || 'MobileShop'}</div>
    ...
    <div>
      <div className="page-header">
        ...
        <div style={{display:'flex',gap:'8px',alignItems:'center'}}>
          ...
          <button className="btn btn-primary" onClick={() => { setForm({...EMPTY_FORM, shop_id: shops.length===1?shops[0].id:(filterShop||'')}); setShowModal(true); }}>
            + New Sale
          </button>
        </div>
      </div>
        ...
        {loading ? <div className="loading">Loading...</div> : (
          <div className="table-wrapper">
            <table>
              <thead>
                <tr>
                  <th>Invoice #</th><th>Customer</th><th>Shop</th><th>Date</th><th>Items</th>
                  <th>Total</th><th>Paid</th><th>Due</th><th>Method</th><th>Status</th><th></th>
                </tr>
              </thead>
              <tbody>
                {filtered.length === 0 ? (
                  <tr><td colSpan={11}><div className="empty-state"><p>No sales yet</p></div></td></tr>
                ) : filtered.map(s => (
                  <tr key={s.id}>
          ...
          <div className="modal" style={{maxWidth:'650px'}} onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-body">
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'0.5rem',marginBottom:'1rem',fontSize:'0.9rem'}}>
                ...
                <div><strong>Sold by:</strong> {viewSale.sold_by || '—'}</div>
