...
const EMPTY_FORM = () => ({
  ...
  items: [{ product_id: '', product_name: '', serial_number: '', qty: 1, unit_price: '', recommended_price: '', unit_cost: '', discount: 0 }],
});
...
const paymentColor = (method) => ({
  cash: '#059669', card: '#2563eb', bank_transfer: '#7c3aed',
  tabby: '#0ea5e9', tamara: '#f59e0b', pending: '#dc2626',
}[method] || '#6b7280');

const statusColor = (s) => ({
  paid: '#059669', partial: '#d97706', unpaid: '#dc2626',
  returned: '#6b7280', payment_pending: '#2563eb',
}[s] || '#6b7280');
...
function printInvoice(sale) {
  ...
  <div class="header">
    ...
    <div><strong>${sale.invoice_number}</strong></div><div>Date: ${fmtDate(sale.sale_date)}</div>
    ${sale.is_exchange ? '<div style="color:#f59e0b;font-weight:bold">EXCHANGE</div>' : ''}
    </div>
  </div>
  ...
  ${sale.is_exchange?`<div style="background:#fef3c7;padding:12px;border-radius:8px;margin-bottom:16px;font-size:13px">
    <strong>Exchange:</strong> ${sale.exchange_product_name||''} (S/N: ${sale.exchange_serial_number||''}) — Trade-in value: AED ${Math.round(sale.exchange_trade_in_value||0).toLocaleString()}</div>`:''}
  <div class="totals">
...
export default function Sales() {
  ...
  // Serial number search for a line item
  ...
  const selectSerialProduct = (idx, product) => {
    ...
    setForm({ ...form, items });
    setSerialSearches(prev => ({ ...prev, [idx]: product.serial_number || '' }));
    setSerialResults(prev => ({ ...prev, [idx]: [] }));
  };

  const addItem = () => setForm(f => ({
    ...f, items: [...f.items, { product_id: '', product_name: '', serial_number: '', qty: 1, unit_price: '', recommended_price: '', unit_cost: '', discount: 0 }]
  }));
  ...
  const updateItem = (i, key, val) => {
    ...
    if (key === 'product_id') {
      ...
      if (p) {
        ...
        setSerialSearches(prev => ({ ...prev, [i]: p.serial_number || '' }));
      }
    }
    setForm(f => ({ ...f, items }));
  };
  ...
  const handleReturn = async () => {
    ...
    try {
      await api.post(`/sales/${returnInvoice.id}/return`, {
        note: returnNote,
    ...
    <div>
      <div className="page-header">
        ...
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <button className="btn btn-ghost" style={{ color: '#dc2626' }} onClick={() => setShowReturn(true)}>
            🔄 Return
          </button>
                ...
                {filtered.length === 0 ? (
                  ...
                  <tr key={s.id}>
                    <td>
                      ...
                      {s.is_exchange && <span style={{ marginLeft: '4px', fontSize: '.7rem', background: '#fef3c7', color: '#92400e', padding: '1px 5px', borderRadius: '6px' }}>EXCHANGE</span>}
                    </td>
                    <td>
                    ...
                    <td>
                      <span style={{ fontSize: '.78rem', fontWeight: 600, color: paymentColor(s.payment_method), textTransform: 'uppercase' }}>
                        {s.payment_method}
                      </span>
                    </td>
                    <td>
                      <span style={{ fontSize: '.78rem', fontWeight: 600, color: statusColor(s.payment_status), textTransform: 'uppercase' }}>
                      ...
                      <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap' }}>
                        ...
                        {s.payment_status === 'payment_pending' && (
                          <button
                            ...
                            style={{ background: '#d1fae5', color: '#065f46', border: 'none', cursor: 'pointer', fontSize: '.75rem', padding: '3px 8px', borderRadius: '6px' }}
            ...
            <div className="modal-body">
              ...
              <div className="form-grid">
                ...
                <div className="form-group">
                  ...
                  <input className="form-control" placeholder="+971XXXXXXXX"
              ...
              {form.payment_method === 'pending' && (
                <div className="form-group" style={{ background: '#fef2f2', padding: '12px', borderRadius: '8px', marginBottom: '12px' }}>
                  <label className="form-label">Pending Amount (AED) — how much is outstanding?</label>
              ...
              {['tabby', 'tamara', 'card', 'bank_transfer'].includes(form.payment_method) && (
                <div style={{ padding: '10px 14px', background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: '8px', marginBottom: '12px', fontSize: '.85rem', color: '#1e40af' }}>
                  ℹ️ <strong>{form.payment_method.toUpperCase()}</strong> — Invoice will show as "Awaiting Payment". Accountant clicks <strong>Mark Received</strong> when money arrives in account.
                </div>
              )}
              ...
              {form.items.map((item, i) => (
                <div key={i} style={{ background: 'var(--bg-secondary)', borderRadius: '8px', padding: '0.75rem', marginBottom: '0.5rem' }}>
                  {/* Serial number search — primary way to add item */}
                  <div style={{ marginBottom: '8px', position: 'relative' }}>
                    ...
                    {(serialResults[i] || []).length > 0 && (
                      <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, zIndex: 100, background: 'white', border: '1px solid var(--border)', borderRadius: '8px', boxShadow: '0 4px 12px rgba(0,0,0,.1)', maxHeight: '200px', overflowY: 'auto' }}>
                        {serialResults[i].map(p => (
                          <div key={p.id}
                            onClick={() => selectSerialProduct(i, p)}
                            ...
                            <span style={{ color: 'var(--text-muted)', marginLeft: '8px', fontSize: '.8rem' }}>S/N: {p.serial_number}</span>
                            <span style={{ marginLeft: '8px', color: '#059669' }}>AED {Math.round(p.selling_price || 0).toLocaleString()}</span>
                          </div>
                        ))}
                      </div>
                    )}
              ...
              <div style={{ background: form.is_exchange ? '#fef9c3' : 'var(--bg-secondary)', border: `1px solid ${form.is_exchange ? '#fde68a' : 'var(--border)'}`, borderRadius: '8px', padding: '12px', marginBottom: '12px' }}>
                <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', fontWeight: 600 }}>
                  <input type="checkbox" checked={form.is_exchange}
                  ...
                  <div className="form-grid" style={{ marginTop: '12px' }}>
                    <div className="form-group">
                      ...
                      <input className="form-control" value={form.exchange_product_name}
                        onChange={e => setForm({ ...form, exchange_product_name: e.target.value })}
                        placeholder="e.g. iPhone X 64GB Black" />
                    </div>
                    <div className="form-group">
                      <label className="form-label">Serial / IMEI of trade-in phone</label>
                      <input className="form-control" value={form.exchange_serial_number}
                        onChange={e => setForm({ ...form, exchange_serial_number: e.target.value })}
                    ...
                    <div className="form-group">
                      ...
                      <input type="number" className="form-control" value={form.exchange_trade_in_value}
                        onChange={e => setForm({ ...form, exchange_trade_in_value: e.target.value })}
                        placeholder="0" />
                    </div>
                  </div>
                )}
          ...
          <div className="modal" style={{ maxWidth: '560px' }} onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-body">
              <div className="form-group" style={{ marginBottom: '12px', position: 'relative' }}>
                ...
                <input className="form-control"
                  placeholder="Type invoice number or serial number..."
                  value={returnSearch}
                  onChange={e => searchForReturn(e.target.value)} />
                {returnResults.length > 0 && (
                  <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, zIndex: 100, background: 'white', border: '1px solid var(--border)', borderRadius: '8px', boxShadow: '0 4px 12px rgba(0,0,0,.1)' }}>
                    {returnResults.map(inv => (
                      <div key={inv.id} onClick={() => selectReturnInvoice(inv)}
                        ...
                        <span style={{ marginLeft: '8px', fontSize: '.78rem', color: 'var(--text-muted)' }}>{fmtDate(inv.sale_date)}</span>
                      </div>
                    ))}
                  </div>
                )}
                ...
                <>
                  <div style={{ padding: '12px', background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: '8px', marginBottom: '16px', fontSize: '.9rem' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4px' }}>
                      ...
                      <div>Date: <strong>{fmtDate(returnInvoice.sale_date)}</strong></div>
                      <div>Original Total: <strong style={{ color: '#059669' }}>{fmt(returnInvoice.total_amount)}</strong></div>
                      <div>Amount Paid: <strong>{fmt(returnInvoice.amount_paid)}</strong></div>
                    </div>
                    {(returnInvoice.items || []).length > 0 && (
                      <div style={{ marginTop: '8px', fontSize: '.82rem', color: 'var(--text-muted)' }}>
                        Items: {returnInvoice.items.map(i => `${i.product_name}${i.serial_number ? ` (${i.serial_number})` : ''}`).join(', ')}
                      </div>
                    )}
                  </div>
                  ...
                  <div className="form-group" style={{ marginBottom: '12px' }}>
                    ...
                    {returnAmount && parseFloat(returnAmount) < parseFloat(returnInvoice.amount_paid || 0) && (
                      <div style={{ marginTop: '4px', fontSize: '.82rem', color: '#d97706' }}>
                        ⚠️ Deduction: AED {Math.round(parseFloat(returnInvoice.amount_paid) - parseFloat(returnAmount)).toLocaleString()} kept from original payment
                      </div>
                    )}
                  </div>
            ...
            <div className="modal-footer">
              ...
              <button className="btn btn-primary" style={{ background: '#dc2626' }}
                onClick={handleReturn} disabled={!returnInvoice}>
                Confirm Return
              </button>
            </div>
          </div>
          ...
          <div className="modal" style={{ maxWidth: '650px' }} onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-footer">
              ...
              {viewSale.payment_status === 'payment_pending' && (
                <button className="btn btn-ghost" style={{ color: '#059669' }} onClick={() => { markReceived(viewSale.id); setViewSale(null); }}>
