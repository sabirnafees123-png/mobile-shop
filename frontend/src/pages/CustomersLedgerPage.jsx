...
function ReceiptModal({ customer, onClose, onSaved }) {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      await apiFetch(`/customers/${customer.id}/receipts`, {
        method: "POST",
          ...
          <div className="form-grid">
            ...
            <div className="form-group">
              ...
              <input type="text" value={form.note}
                ...
                placeholder="Optional note" className="form-input" />
            </div>
          </div>
        </div>
          ...
          <table className="ledger-table">
            <thead>
              <tr>
                ...
                <th>Reference</th>
                <th>Note</th>
                <th className="num">Debit (Sale)</th>
                <th className="num">Credit (Receipt)</th>
            ...
            <tbody>
              {data.ledger.map((row, i) => (
                <tr key={i} className={`row-${row.type}`}>
                  ...
                  <td>
                    {row.type === "receipt" && (
                      <button className="btn-icon danger" onClick={() => handleDelete(row.ref_id)} title="Delete">🗑</button>
                    )}
                  </td>
                </tr>
              ))}
...
// ─── Customers List ───────────────────────────────────────────────────────────
function CustomersList({ onSelect, onReceipt }) {
            ...
            <tbody>
              {filtered.map((c, i) => (
                <tr key={c.id}>
                  <td>{i + 1}</td>
                  <td>
                    <button className="link-btn" onClick={() => onSelect(c)}>{c.name}</button>
                  </td>
                  <td>{c.phone || "—"}</td>
                  <td className="num">{fmt(c.total_sales)}</td>
                  <td className="num">{fmt(c.total_received)}</td>
                  <td className="num">
                    <span className={`balance-pill ${Number(c.balance) > 0 ? "owing" : "clear"}`}>
                      {fmt(c.balance)}
                    </span>
                  </td>
                  <td>
                    <div className="action-btns">
                      ...
                      <button className="btn btn-sm btn-pay"
                        onClick={() => onReceipt(c)}
...
export default function CustomersLedgerPage() {
  ...
  const openLedger  = (c) => { setSelected(c); setView("ledger"); };
  const openReceipt = (c) => setReceiptCustomer(c);
  const closeReceipt = () => setReceiptCustomer(null);
  const afterReceipt = () => { closeReceipt(); setRefreshKey((k) => k + 1); };
  ...
  return (
    <>
      ...
      <div className="sl-root">
        {view === "list" && (
          <CustomersList key={refreshKey} onSelect={openLedger} onReceipt={openReceipt} />
        )}
...
const CSS = `
  ...
  .page-header, .ledger-header {
    ...
    padding: 20px 24px; border-bottom: 1px solid #e8eaf0; background: #fff;
  }
  .page-header h2, .ledger-header h2 { margin: 0; font-size: 1.4rem; }
  .header-badge { margin-left: auto; background: #dbeafe; color: #1e40af; padding: 6px 14px; border-radius: 20px; font-size: .88rem; }
  .ledger-title { flex: 1; }
  ...
  .form-input { padding: 8px 12px; border: 1px solid #d1d5db; border-radius: 6px; font-size: .9rem; outline: none; background: #fff; }
